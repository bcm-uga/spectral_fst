...
Here, we use a F model with 3 populations to simulate our datas. The simulated datas has been obtained using the function fmodel_simulation with default parameters *fmodel_simulation(n=150, L=20000, c_vector=c(1/3,1/3,1/3), F_vector=c(0.3,0.2,0.1))*
```{r}
data('fmodel')
X <- fmodel$genotype
pop <- fmodel$pop
...
```{r}
spectral_adjusted_athaliana <- compute_partition(athaliana$genotype, athaliana$pop, make_adjustment = T, adjusting_variables = athaliana$bio)
