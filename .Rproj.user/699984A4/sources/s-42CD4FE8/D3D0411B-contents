...
#' eigenZs : The biggest eigenvalue of Zs
...
compute_partition <- function(genotype, population_assignment){
  ...
  unique_pop <- unique(population_assignment)
  ...
  pc_z <- prcomp(genotype, scale=T)
  pc_z_st <- prcomp(Zst, scale=F)
  pc_z_s <- prcomp(Zs, scale=F)


  Fst <- sum(pc_z_st$sdev^2 / L)
  Fst_estimate <- sum((pc_z$sdev^2 / L)[1:nb_pop])
  eigenZs <- (pc_z_s$sdev^2 / L)[1]
  ...
  return(list(Zs = Zs, Zst=Zst, Fst=Fst, Fst_estimate=Fst_estimate, eigenZs=eigenZs, MPestimate=MPestimate))
}
